...
RSpec.describe External::Comics::Character::Get, type: :request do
  subject(:do_request) { External::Comics::Character::Get.new(name:, page:, user_id:).call }
  let(:name) { 'deadpool' }
  let(:page) { 0 }
  let(:user_id) { 3231 }
  ...
  describe 'Makes a successful request to get comics' do
    before do
      allow(External::Comics::Character::Get).to receive(:new).and_return(
        instance_double(External::Comics::Character::Get, call: OpenStruct.new(success?: true, result: [ id: character_id ])
      ))
    end
